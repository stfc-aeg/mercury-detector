...
const std::string MercuryFrameDecoder::CONFIG_SENSORS_LAYOUT = "sensors_layout";
	...
	fem_packets_lost_(0)
{
  ...
  dropped_frame_buffer_.reset(new uint8_t[Mercury::max_frame_size(sensors_config_)]);
...
const size_t MercuryFrameDecoder::get_frame_buffer_size(void) const
{
  size_t frame_buffer_size = get_frame_header_size() +
      Mercury::frame_size(sensors_config_);
...
void MercuryFrameDecoder::initialise_frame_header(Mercury::FrameHeader* header_ptr)
{
  ...
  //memset(header_ptr->fem_rx_state, 0, sizeof(Mercury::FemReceiveState));	//  No longer an array, do not need to copy 0's into it
...
void* MercuryFrameDecoder::get_next_payload_buffer(void) const
{
  ...
  if (current_packet_fem_map_.fem_idx_ != ILLEGAL_FEM_IDX)
  {
    std::size_t frame_size = Mercury::frame_size(sensors_config_);
...
size_t MercuryFrameDecoder::get_next_payload_size(void) const
{
  ...
  if (get_packet_number() < Mercury::num_primary_packets[sensors_config_])
  {
    next_receive_size = Mercury::primary_packet_size;
  }
  else
  {
    next_receive_size = Mercury::tail_packet_size[sensors_config_];
  }
		...
		int port, struct sockaddr_in* from_addr)
{
  ...
  if (current_packet_fem_map_.fem_idx_ != ILLEGAL_FEM_IDX)
  {
    ...
    if (current_frame_header_->total_packets_received == Mercury::num_fem_frame_packets(sensors_config_))
    {
  ...
  while (buffer_map_iter != frame_buffer_map_.end())
  {
    ...
    if (elapsed_ms(frame_header->frame_start_time, current_time) > frame_timeout_ms_)
    {
      ...
      const std::size_t num_fem_frame_packets =
          Mercury::num_fem_frame_packets(sensors_config_);
...
//!
void MercuryFrameDecoder::parse_sensors_layout_map(const std::string sensors_layout_str)
{
    // Clear the current map
    ...
    if (map_entries.size() == 2) {
        ...
        sensors_layout_[0] = MercurySensorLayoutMapEntry(sensor_rows, sensor_columns);
    }
    ...
    {
      sensors_config_ = Mercury::sensorConfigOne;			// 1 x 1 sensors
    }
    else if ((sensors_layout_[0].sensor_rows_ == 2) && (sensors_layout_[0].sensor_columns_ == 2))
    {
      sensors_config_ = Mercury::sensorConfigTwo;     // 2 x 2 sensors
    }
}
